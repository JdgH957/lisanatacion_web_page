...
from app.Domain.mappers.entrenador_mapper import actualizar_entrenador_con_dto, dominio_a_dto_out, dto_a_dominio
...
class EntrenadorService(IEntrenadorService):
    ...
    def actualizar_entrenador(
            self, entrenador_id: int,
            entrenador_dto: EntrenadorUpdateDTO) -> EntrenadorOutDTO:
        entrenador = self.entrenador_repo.get_entrenador_by_id(entrenador_id)
        if not entrenador:
            raise ValueError("Entrenador no encontrado")
        entrenador_actualizado = actualizar_entrenador_con_dto(entrenador, entrenador_dto)
        entrenador_actualizado = self.entrenador_repo.actualizar_entrenador(
            entrenador_id,
